...
#define ALIGN_ROTATE_AXIS uint(4)
#define ALIGN_LOCAL_BILLBOARD uint(5)

#define CUSTOM_SRC_NONE uint(0)
#define CUSTOM_SRC_X uint(1)
#define CUSTOM_SRC_Y uint(2)
#define CUSTOM_SRC_Z uint(3)
#define CUSTOM_SRC_W uint(4)

#define ALIGN_AXIS_X uint(1)
#define ALIGN_AXIS_Y uint(2)
#define ALIGN_AXIS_Z uint(3)
...
void main() {
	...
	if (bool(floatBitsToUint(velocity_flags.w) & PARTICLE_FLAG_ACTIVE)) {
		...
		if (align_mode == ALIGN_DISABLED) {
			...
			float angle = 0.;
			if (align_custom_src == CUSTOM_SRC_NONE) {
				...
				if (align_custom_src == CUSTOM_SRC_X) {
					angle = custom.x;

				} else if (align_custom_src == CUSTOM_SRC_Y) {
					angle = custom.y;

				} else if (align_custom_src == CUSTOM_SRC_Z) {
					angle = custom.z;

				} else if (align_custom_src == CUSTOM_SRC_W) {
					angle = custom.w;
				}
